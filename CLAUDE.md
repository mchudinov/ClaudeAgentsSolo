...
- **`DeveloperAgent/`** — `Microsoft.NET.Sdk.Web` Blazor Server app, the agent runtime (lifecycle loop, Dapr workflow + actors, developer + reviewer agents, sandbox, operator dashboard). Contains `Dockerfile`, `appsettings.json`, `Components/` (Razor); has a `MudBlazor` dependency. References `Agent.GitHub` and `Agent.Memory`.
...
- **`Agent.Memory/`** — reusable, **agent-neutral** agent-memory library: MAF `AIContextProvider`/`ChatHistoryProvider` (layer 3) plus Dapr-backed stores for chat-history windowing, durable repo/task memories, and agent-session progress, with `ISummarizer`/`IMemoryExtractor` seams the host fills. The deliverable of the memory extraction (Step-40); see "Memory layer" below. Depends only on `Microsoft.Agents.AI` + `Microsoft.Extensions.AI.Abstractions` + `Dapr.Client`; has **no** dependency on `DeveloperAgent`.
...
Tests under `src/Tests/`: **`DeveloperAgent.Tests/`** (app: lifecycle, workflow, agents, sandbox, dashboard, the GitHub facade, integration), **`Agent.GitHub.Tests/`** (the GitHub library's generic mechanics), and **`Agent.Memory.Tests/`** (the memory library's stores, providers, and `AddAgentMemoryServices` registration). The memory subsystem's Dapr round-trip **integration** tests stay in `DeveloperAgent.Tests/Integration/` — they exercise the host's `agent-state-store` component wiring and depend on its `EnvironmentSkip`/`SkippableFact` infra. Integration tests carry `[Trait("Category", "Integration")]` and are excluded by the fast filter (`--filter "Category!=Integration"`); a convention test enforces the trait. CI lives in `.github/workflows/`.
