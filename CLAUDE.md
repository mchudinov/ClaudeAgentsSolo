...
- **`DeveloperAgent/`** — `Microsoft.NET.Sdk.Web` Blazor Server app, the agent runtime (lifecycle loop, Dapr workflow + actors, developer + reviewer agents, sandbox, operator dashboard). Contains `Dockerfile`, `appsettings.json`, `Components/` (Razor); has a `MudBlazor` dependency. References `ClaudeAgents.GitHub`.
- **`ClaudeAgents.GitHub/`** — reusable, **agent-neutral** GitHub Projects v2 + pull-request access library (Octokit + Octokit.GraphQL). The deliverable of the GitHub extraction; see "GitHub access layer" below. Has **no** dependency on `DeveloperAgent`.
...
**Adding GitHub code:** a generic GitHub-API capability → `ClaudeAgents.GitHub` (keep it status-name-keyed and policy-free). Developer-agent lifecycle or PR-template behavior → `DeveloperAgent`. App consumers inject the typed `IGitHubProjectService` facade and keep working in `ProjectState`; a `global using ClaudeAgents.GitHub` in `DeveloperAgent` surfaces the library types without per-file `using`s.
